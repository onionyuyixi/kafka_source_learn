 ...
 */
//一个broker的所有生命周期的掌管
//构造器设计的元素  kafkaconfig  时间  线程的前缀名  kafka度量指数
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None,
  ...
  private val startupComplete = new AtomicBoolean(false)  // 完全启动
  private val isShuttingDown = new AtomicBoolean(false)  //是否关闭
  private val isStartingUp = new AtomicBoolean(false)  //是否正在启动中

  private var shutdownLatch = new CountDownLatch(1)// 为什么这里用到 倒计数器
